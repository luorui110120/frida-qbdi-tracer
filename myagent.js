...
class QBDIBinder extends Binder {
    ...
    async load() {
        return super.load(this.QBDI_LIB, await this.QBDI_PATHS());
    }
}
...
function warp_vm_run(vm_run_func, log_file_path) {
    // let baselibEncryptor = Module.findBaseAddress("libanti-frida.so");
    // let addr_2BD8 = baselibEncryptor.add(0xC3C + 1);
    var addr_2BD8 = Module.findExportByName("libanti-frida.so", "encryptData");
    let str0 = "123";
    let arg0 = Memory.allocUtf8String(str0);
    let ret_len = str0.length;
     ...
     */
    let ret = vm_run_func(addr_2BD8, [arg0, str0.length], log_file_path)

    printMemHex(ret, ret_len)
}

function printMemHex(addr, size){
    var buffer = Memory.readByteArray(ptr(addr), size);
    console.log("============================")
    console.log("BaseAddr:0x"+addr.toString(16)+" size:0x"+size.toString(16))
